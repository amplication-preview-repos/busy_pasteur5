/*
------------------------------------------------------------------------------ 
This code was generated by Amplication. 
 
Changes to this file will be lost if the code is regenerated. 

There are other ways to to customize your code, see this doc to learn more
https://docs.amplication.com/how-to/custom-code

------------------------------------------------------------------------------
  */
import { ArgsType, Field } from "@nestjs/graphql";
import { ApiProperty } from "@nestjs/swagger";
import { UserLoginWhereInput } from "./UserLoginWhereInput";
import { IsOptional, ValidateNested, IsInt } from "class-validator";
import { Type } from "class-transformer";
import { UserLoginOrderByInput } from "./UserLoginOrderByInput";

@ArgsType()
class UserLoginFindManyArgs {
  @ApiProperty({
    required: false,
    type: () => UserLoginWhereInput,
  })
  @IsOptional()
  @ValidateNested()
  @Field(() => UserLoginWhereInput, { nullable: true })
  @Type(() => UserLoginWhereInput)
  where?: UserLoginWhereInput;

  @ApiProperty({
    required: false,
    type: [UserLoginOrderByInput],
  })
  @IsOptional()
  @ValidateNested({ each: true })
  @Field(() => [UserLoginOrderByInput], { nullable: true })
  @Type(() => UserLoginOrderByInput)
  orderBy?: Array<UserLoginOrderByInput>;

  @ApiProperty({
    required: false,
    type: Number,
  })
  @IsOptional()
  @IsInt()
  @Field(() => Number, { nullable: true })
  @Type(() => Number)
  skip?: number;

  @ApiProperty({
    required: false,
    type: Number,
  })
  @IsOptional()
  @IsInt()
  @Field(() => Number, { nullable: true })
  @Type(() => Number)
  take?: number;
}

export { UserLoginFindManyArgs as UserLoginFindManyArgs };
